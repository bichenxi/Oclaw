# Oclaw - 沉浸式 AI 浏览器项目蓝图与实施路线图

## 1. 项目愿景与核心价值

**Oclaw** 的定位不再是一个普通的“套壳浏览器”，而是**为 OpenClaw 量身定制的“沉浸式 AI 浏览器”**。
...
通过 Tauri 的系统级能力、定制化的多子 Webview 架构，结合 OpenClaw 的智能决策，Oclaw 将实现**“人机协同”、“全透明执行”与“安全护城河”**。
...
| 优先级 | 目标 | 说明 |
|--------|------|------|
| **1** | **OpenClaw 控制 Oclaw** | 本应用起小型 HTTP API（如 `127.0.0.1:18790`），提供 `navigate`、`snapshot`、`click` 等；在 OpenClaw 侧增加 skill 调用该 API，使 AI 操作本应用内的 webview，实现「一个窗口、全透明、可接管」。 |
...
> **结语**：不要一开始就做大而全。先用最简单的 HTML 页面跑通 **阶段 0** 的全链路，再逐步攻克复杂第三方网页。Oclaw 的最终形态，将是未来 AI 时代的标准浏览器范式。