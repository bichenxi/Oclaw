# OpenClaw 与 Claw Browser 的集成方向

**结论：不以「Claw Browser 连接 OpenClaw Gateway」为主，而是以「OpenClaw 控制 Claw Browser」为正道。**
...
2. **OpenClaw 侧增加 skill**  
   在 OpenClaw 的 skill 目录下新增一个技能，该技能在「打开网页」「点击」「取快照」等意图时，去请求上述 API，从而**由 OpenClaw 驱动 Claw Browser 内的网页**。

3. **效果**  
   用户在一个窗口内：左侧是 AI 控制台（可后续从 OpenClaw 拉思考链或仅做本地展示），右侧是 Claw Browser 的 webview，**所有浏览器操作都由 OpenClaw 通过 API 控制**，实现「一个窗口、全透明、可接管」。
...
- 编写并配置 OpenClaw 的「Claw Browser」skill，在技能内按需请求上述 API。
