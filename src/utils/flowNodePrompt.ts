...
/**
 * 为团队方案（多智能体编排）中的单个 Agent 节点拼装发给模型的提示词。
 * 若节点配置了 flow_role，会作为「团队内职责」注入，便于协作时各 Agent 明确本步交付物。
 */
export function buildFlowNodePrompt(
  ...
  if (duty) {
    parts.push(
      `【本节点在团队方案中的职责】\n${duty}\n\n`
      + `你是本团队方案中的节点「${node.label}」（使用 Agent「${agentName}」）。请严格按上述职责与【总体任务】完成本步应交付的内容；输出应便于下游节点继续处理。`,
    )
  } else {
    parts.push(
      `【你的角色】\n你是「${node.label}」（Agent：${agentName}）。请围绕【总体任务】与上游输出，完成本团队方案中这一环应交付的内容。`,
    )
  }
